...
import type { Alarm, ToneId, RepeatMode } from '../lib/useAlarm'
import { playTone } from '../lib/useAlarm'

export const TONES: { id: ToneId; label: string; emoji: string }[] = [
...
export const repeatLabel = (r: RepeatMode) =>
  r === 'once' ? 'Once' : r === 'daily' ? 'Daily' : 'Weekdays'
...
interface AlarmModalProps {
  ...
  snoozed: boolean
  cancelSnooze: () => void
  previewTone: (id: ToneId) => void
}

export default function AlarmModal({
  isOpen, onClose, alarms, setAlarms, snoozed, cancelSnooze, previewTone
}: AlarmModalProps) {
  ...
  const openNew = () => {
    ...
    setEditing(-1); setEditH12(7); setEditAmPm('AM')
    setEditMinute(0); setEditLabel(''); setEditTone('pulse'); setEditRepeat('once')
  }

  const openEdit = (a: Alarm) => {
    const { h12, ampm } = to12(a.hour)
    setEditing(a.id); setEditH12(h12); setEditAmPm(ampm)
    setEditMinute(a.minute); setEditLabel(a.label)
    setEditTone(a.tone); setEditRepeat(a.repeat)
  }
  ...
  const toggleAlarm = (id: number) => setAlarms(prev => prev.map(a =>
    a.id === id ? { ...a, enabled: !a.enabled, fired: false } : a
  ))
              ...
              <AnimatePresence>
                {snoozed && (
                  <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="mb-3 overflow-hidden">
                    <div className="rounded-2xl px-4 py-3 flex items-center justify-between" style={{ background: 'rgba(37,99,235,0.08)', border: '1px solid rgba(37,99,235,0.2)' }}>
                      <p className="text-[11px] font-display font-bold" style={{ color: '#3B82F6' }}>⏱ Snoozed · Re-fires in 5 min</p>
                      <motion.button whileTap={{ scale: 0.9 }} onClick={cancelSnooze}
                    ...
                    <motion.div key={a.id} layout
                      ...
                      <motion.button whileTap={{ scale: 0.97 }} onClick={() => openEdit(a)} className="flex-1 text-left">
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          ...
                          <span className="text-[9px] font-display font-bold px-1.5 py-0.5 rounded-md" style={{ background: isDark ? 'rgba(255,255,255,0.06)' : '#F1F5F9', color: textSecondary }}>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <motion.button whileTap={{ scale: 0.9 }} onClick={() => toggleAlarm(a.id)}
                          ...
                          <motion.div animate={{ x: a.enabled ? 20 : 2 }} transition={{ type: 'spring', stiffness: 400, damping: 30 }}
              ...
              <AnimatePresence>
                {editing !== null && (
                  <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 20 }}
                    ...
                    <div className="flex items-center justify-center gap-3 mb-4">
                      <div className="flex flex-col items-center gap-1">
                        <motion.button whileTap={{ scale: 0.85 }} onClick={() => setEditH12(h => h === 12 ? 1 : h + 1)}
                          className="w-10 h-8 rounded-xl flex items-center justify-center" style={{ background: isDark ? 'rgba(255,255,255,0.06)' : '#E2E8F0' }}>
                      ...
                      <div className="flex flex-col items-center gap-1">
                        <motion.button whileTap={{ scale: 0.85 }} onClick={() => setEditMinute(m => (m + 1) % 60)}
                          className="w-10 h-8 rounded-xl flex items-center justify-center" style={{ background: isDark ? 'rgba(255,255,255,0.06)' : '#E2E8F0' }}>
                          <svg width="12" height="12" viewBox="0 0 24 24" fill="none"><path d="M18 15l-6-6-6 6" stroke={textSecondary} strokeWidth="2" strokeLinecap="round" /></svg>
                        </motion.button>
                        ...
                        <motion.button whileTap={{ scale: 0.85 }} onClick={() => setEditMinute(m => (m - 1 + 60) % 60)}
                          className="w-10 h-8 rounded-xl flex items-center justify-center" style={{ background: isDark ? 'rgba(255,255,255,0.06)' : '#E2E8F0' }}>
                          <svg width="12" height="12" viewBox="0 0 24 24" fill="none"><path d="M6 9l6 6 6-6" stroke={textSecondary} strokeWidth="2" strokeLinecap="round" /></svg>
                        </motion.button>
                      </div>
                    ...
                    <input type="text" placeholder="Label (e.g. Med Pass, Break End)"
                    ...
                    <div className="flex gap-2 mb-3">
                      {REPEATS.map(r => (
                        <motion.button key={r.id} whileTap={{ scale: 0.9 }} onClick={() => setEditRepeat(r.id)}
                          ...
                          style={{ background: editRepeat === r.id ? 'rgba(37,99,235,0.15)' : (isDark ? 'rgba(255,255,255,0.05)' : '#F1F5F9'), border: editRepeat === r.id ? '1px solid rgba(37,99,235,0.3)' : '1px solid transparent', color: editRepeat === r.id ? '#3B82F6' : textSecondary }}>
                          {r.label}
                        </motion.button>
                      ))}
                    </div>
                    ...
                    <div className="flex gap-2 flex-wrap mb-4">
                      {TONES.map(t => (
                        <motion.button key={t.id} whileTap={{ scale: 0.9 }}
                          ...
                          style={{ background: editTone === t.id ? 'rgba(37,99,235,0.15)' : (isDark ? 'rgba(255,255,255,0.05)' : '#F1F5F9'), border: editTone === t.id ? '1px solid rgba(37,99,235,0.3)' : '1px solid transparent', color: editTone === t.id ? '#3B82F6' : textSecondary }}>
