...
function MainApp() {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center" style={{ height: '100dvh', background: '#050912' }}>
        <div className="flex flex-col items-center gap-3">
          <h1 className="font-display font-extrabold text-2xl text-white tracking-tight">eTally</h1>
          <div style={{ width: 24, height: 24, border: '2px solid rgba(255,255,255,0.1)', borderTopColor: '#3B82F6', borderRadius: '50%', animation: 'spin 0.7s linear infinite' }} />
    ...
    return (
      <div className="flex flex-col items-center justify-center" style={{ height: '100dvh', background: '#050912' }}>
        <div className="flex flex-col items-center gap-4 px-6 w-full max-w-sm">
          <div className="w-16 h-16 rounded-2xl flex items-center justify-center" style={{ background: 'rgba(37,99,235,0.1)', border: '1px solid rgba(37,99,235,0.2)' }}>
  ...
  return (
    <div className="flex flex-col" style={{ height: '100dvh', overflow: 'hidden' }} data-theme={theme}>
        ...
        <div className="flex items-center justify-between">
          ...
          >
            <div className="flex items-center gap-2">
              <h1 className="font-display font-extrabold text-2xl text-white tracking-tight leading-none">eTally</h1>
              <span className="text-[9px] font-display font-bold tracking-widest text-slate-500 bg-slate-800 px-2 py-0.5 rounded-lg">v2.0</span>
            </div>
            <p className="font-body text-[11px] mt-0.5 tracking-wide" style={{ color: isDark ? '#3B82F6' : '#64748B' }}>
          ...
          <div className="flex items-center gap-1.5">
            ...
            {/* Vault */}
            <motion.button whileTap={{ scale: 0.88 }} onClick={() => setVaultOpen(true)}
              initial={{ opacity: 0, scale: 0.7 }} animate={{ opacity: 1, scale: 1 }}
            ...
            {/* Help */}
            <motion.button whileTap={{ scale: 0.88 }} onClick={() => setAboutOpen(true)}
              initial={{ opacity: 0, scale: 0.7 }} animate={{ opacity: 1, scale: 1 }}
            ...
            {/* About Developer */}
            <motion.button whileTap={{ scale: 0.88 }} onClick={() => setCreditOpen(true)}
              initial={{ opacity: 0, scale: 0.7 }} animate={{ opacity: 1, scale: 1 }}
            ...
            {/* Alarm */}
            <motion.button whileTap={{ scale: 0.88 }} onClick={() => setAlarmOpen(true)}
              initial={{ opacity: 0, scale: 0.7 }} animate={{ opacity: 1, scale: 1 }}
            ...
            {/* Sign Out */}
            <motion.button whileTap={{ scale: 0.88 }} onClick={handleSignOut}
              initial={{ opacity: 0, scale: 0.7 }} animate={{ opacity: 1, scale: 1 }}
        ...
        {firing && (
          ...
          >
            <div className="rounded-3xl px-6 py-8 text-center w-full max-w-sm"
              ...
              <motion.div animate={{ scale: [1, 1.15, 1] }} transition={{ repeat: Infinity, duration: 1 }} className="text-5xl mb-4">⏰</motion.div>
      ...
      <AlarmModal isOpen={alarmOpen} onClose={() => setAlarmOpen(false)} alarms={alarms} setAlarms={setAlarms} snoozed={snoozed} cancelSnooze={cancelSnooze} previewTone={previewTone} />
        ...
        {creditOpen && (
          <>
            ...
            <motion.div initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }}
              ...
              style={{ background: 'linear-gradient(160deg, #050912 0%, #0A1128 60%, #080D1E 100%)', paddingBottom: 'max(2rem, env(safe-area-inset-bottom, 2rem))', borderTop: '1px solid rgba(255,255,255,0.06)' }}>
              <div className="flex justify-center pt-3 pb-2"><div className="w-10 h-1 rounded-full bg-slate-700" /></div>
              <div className="px-6 pb-2 flex flex-col items-center text-center">
                ...
                <motion.button whileTap={{ scale: 0.97 }} onClick={() => setCreditOpen(false)}
